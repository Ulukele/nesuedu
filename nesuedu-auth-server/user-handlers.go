...
func (s *Server) HandleAuthSignUp(c *fiber.Ctx) error {
	...
	var req userSetParamsRequest
	if err := c.BodyParser(&req); err != nil {
		return fiber.NewError(fiber.StatusBadRequest, "expect username, password, firstName, lastName")
	}
	if err := validate.Struct(req); err != nil {
		log.Printf(err.Error())
		return fiber.NewError(fiber.StatusBadRequest, "validation error")
	...
	user, err := s.dbe.CreateUser(req.Username, req.Password, req.FirstName, req.LastName)
	if err != nil {
